...
export function DashboardPage({
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
          ...
          {user?.role === "OWNER" && (
            <>
              <button
                ...
                className="px-3.5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-xs transition flex items-center space-x-1.5"
              >
      ...
      {isDatabaseEmpty && (
        <div className="p-8 rounded-3xl bg-teal-900 text-white border border-teal-700 shadow-lg relative overflow-hidden">
          <div className="max-w-2xl relative z-10 space-y-3">
            ...
            <div className="pt-2">
              <button
                onClick={onAddProductClick}
                className="px-6 py-3 bg-white text-teal-900 hover:bg-teal-50 font-black text-sm rounded-xl shadow-md transition inline-flex items-center space-x-2"
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-2">
          ...
          <div className="text-xs text-slate-500 flex items-center justify-between pt-1 border-t border-slate-100">
            ...
            <span className="font-bold text-slate-700">GH₵ {parseFloat(data?.sales.monthSales || "0").toFixed(2)}</span>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-2">
          ...
          <div className="text-xs text-slate-500 flex items-center justify-between pt-1 border-t border-slate-100">
            ...
            <span className="font-bold text-slate-700">
              {data?.inventory.totalProducts || 0} ({data?.inventory.totalUnits || 0} units)
            </span>
          </div>
        </div>
      ...
      {/* Stock Health & Alerts Overview */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {/* Low Stock Alert Box */}
        <div
          onClick={() => onNavigate("products")}
          className={`p-4 rounded-2xl border shadow-2xs cursor-pointer transition flex items-center justify-between ${
        ...
        >
          <div className="space-y-1">
            ...
            <div className="text-2xl font-black text-amber-700">{effectiveLowStockCount}</div>
        ...
        <div
          ...
          className="bg-white p-4 rounded-2xl border border-rose-200 hover:border-rose-300 shadow-2xs cursor-pointer transition flex items-center justify-between"
        >
          <div className="space-y-1">
            <span className="text-xs font-bold text-rose-900 uppercase">Expiring Batches (&lt; 90d)</span>
            <div className="text-2xl font-black text-rose-700">{data?.inventory.expiringSoonCount || 0}</div>
            <p className="text-[11px] text-slate-500">FEFO priority dispensing required</p>
          </div>
        ...
        <div
          ...
          className="bg-white p-4 rounded-2xl border border-teal-200 hover:border-teal-300 shadow-2xs cursor-pointer transition flex items-center justify-between"
        >
          <div className="space-y-1">
            <span className="text-xs font-bold text-teal-900 uppercase">Active Cashier Shifts</span>
            <div className="text-2xl font-black text-teal-700">{data?.employees.openShifts || 0}</div>
            <p className="text-[11px] text-slate-500">Active terminal registers open</p>
          </div>
